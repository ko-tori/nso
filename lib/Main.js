...
Editor.renderTimeline  = function() {
	...
	if (Editor.timeline2_dragging) {
		...
		let x1 = ((Editor.drag_startt - t) * Editor.options.timelinezoom / 8 + .5) * width,
...
$(document).ready(function main() {
		...
		if (!e.ctrlKey || !clicked) {
			...
			if (clicked) Editor.selected = [clicked];
				...
				var timeline = $('#timeline2'),
					dx = (page_mouseX - timeline.offset().left - timeline.width() / 2),
					pxps = timeline.width() / 8 * Editor.options.timelinezoom;
				Editor.drag_startt = Editor.source.t + dx / pxps;
			}
		} else {
			Editor.selected.push(clicked);
		}
	...
	Editor.updateMouseLocation = function () {
		...
		if (timeline_mousedown)
			...
			if(Editor.edit_dragging) {
				...
				// add more stuff to keep it in the play area
		...
		} else if (timeline2_mousedown) {
			if (Editor.timeline2_dragging) {
				var timeline = $('#timeline2'),
					temp = [];
				function isTimelineScroll() {
					return timeline2_mousedown && !Editor.source.playing && (page_mouseX < timeline.offset().left || page_mouseX > timeline.offset().left + timeline.width());
				}
				if (isTimelineScroll() && !Editor.scroll_interval) {
					Editor.scroll_interval = setInterval(function() {
						if (!isTimelineScroll()) {
							...
							let n = page_mouseX - timeline.offset().left - timeline.width() / 2;
							n -= Math.sign(n) * timeline.width() / 2;
							if (n > 2 * timeline.offset().left) n = 2 * timeline.offset().left;
							Editor.source.t = Math.max(0, Math.min(Editor.source.duration, Editor.source.t + n / 4 / Math.pow(4 + Editor.options.timelinezoom, 2)));
							Editor.updateTime();
							Editor.renderupdate();
						}
					}, 10);
				}
				var dx = (page_mouseX - timeline.offset().left - timeline.width() / 2),
					pxps = timeline.width() / 8 * Editor.options.timelinezoom,
	...
	var captureMouseLocation = function (e) {
		...
		if (timeline_mousedown || edit_mousedown || timeline2_mousedown) mousemoved = true;
		page_mouseX = e.pageX;
		page_mouseY = e.pageY;
		Editor.updateMouseLocation();
	};
