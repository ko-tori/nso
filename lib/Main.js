...
Editor.renderTimeline  = function() {
	...
	/**if (Editor.options.beatsnapdivisor % 16)
	if (Editor.options.beatsnapdivisor % 8)
	if (Editor.options.beatsnapdivisor % 4)
	
	if (Editor.options.beatsnapdivisor % 12)
	if (Editor.options.beatsnapdivisor % 6)
	if (Editor.options.beatsnapdivisor % 3)
	**/
	// if (Editor.options.beatsnapdivisor % 2 == 0) {
	// 	tctx.beginPath();
	// 	for (var x = beat1pos; x < width; x += beatl * Editor.options.timelinezoom / 8 * width) {
	// 		tctx.moveTo(x, height * 0.6);
	// 		tctx.lineTo(x, height - 3);
	// 	}
	// 	tctx.stroke();
