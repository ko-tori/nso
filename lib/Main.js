...
Editor.render = function () {
	...
	var width = tctx.canvas.width;
	var height = tctx.canvas.height;
	tctx.clearRect(0, 0, width, height);
	tctx.save();
	renderFrom = Editor.beatmap.getIndexAt((Editor.source.t - 4 / Editor.options.timelinezoom - Editor.maxsliderdur) * 1000);
	renderTo = Editor.beatmap.getIndexAt((Editor.source.t + 4 / Editor.options.timelinezoom) * 1000);
	for (var i = renderTo; i >= renderFrom; i--) {
		Editor.beatmap.HitObjects[i].timelineRender();
	}
	tctx.restore();
	tctx.strokeStyle = '#FFF';
	tctx.lineWidth = 1;
	tctx.beginPath();
	tctx.moveTo(0, height - 1.5);
	tctx.lineTo(width, height - 1.5);
	tctx.moveTo(width / 2 - 1.5, height / 10);
	tctx.lineTo(width / 2 - 1.5, height - 1.5);
	tctx.moveTo(width / 2 + 1.5, height / 10);
	tctx.lineTo(width / 2 + 1.5, height - 1.5);
	tctx.stroke();
	var fadew = 25;
...
$(document).ready(function main() {
	...
	Editor.getCurObj = function () {
		...
		for (var i = Editor.rendered.length - 1; i >= 0; i--) {
			...
			if (obj instanceof HitCircle || (Editor.rendered.indexOf(obj) == -1 && Editor.selected.indexOf(obj) != -1)) {
	...
	$('#zoomout').click(function () {
		Editor.options.timelinezoom = Math.max(0.1, Editor.options.timelinezoom - 0.1);
		Editor.update();
	});
