...
Editor.renderTimeline  = function() {
	...
	tctx.save();
	var renderFrom = Editor.beatmap.getIndexAt((Editor.source.t - 3 / Editor.options.timelinezoom - Editor.maxsliderdur) * 1000),
		renderTo = Editor.beatmap.getIndexAt((Editor.source.t + 3 / Editor.options.timelinezoom) * 1000);
...
Editor.updateTime = function() {
	...
	$('#time').html(Util.msToTimeString(t * 1000));
...
$(document).ready(function main() {
	...
	Editor.clipboard = new ClipboardManager();
	Editor.selected = [];
	Editor.timeline_rendered = new Set();
	Editor.playfield_rendered = new Set();
			...
			var timeline = $('#timeline2'),
				...
				if (Editor.selected.length < 2 && Editor.timeline2_objend) {
					...
					for (let i = 0; i < Editor.selected.length; i++) {
						...
						// If this is not done, rendering will not work properly.
						// collabTODO
	...
	var keypresshandler = function (e) {
		...
		if ((code == 67 || code == 32) && Editor.source) { //c, space
			if (code == 67 && e.ctrlKey) { //ctrl-c
				Editor.clipboard.copySelectedToClipboard();
			} else if (!e.ctrlKey) {
				if (Editor.source.playing) Editor.source.pause();
				else {
					Editor.source.play();
