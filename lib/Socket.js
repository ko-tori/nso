...
class Socket {
	...
	static get_osu(callback) {
		if (Editor.fileInfo && Editor.fileInfo.difficulty) {
			...
			alert('Room does not exist. Redirecting to homepage...'); //TODO: make better modal or something
	...
	constructor() {
		...
		socket.on("connect", function () {
			...
			socket.on('edit move t', function (data) {
				//var obj = beatmap.matchObj(data[0]);
				var obj = Editor.beatmap.HitObjects[Editor.beatmap.getIndexAt(HitObject.parse(data[0]).startTime)];
				if (obj) {
					// TODO
			...
			socket.on('user t', function (data) {
	...
	edit_movet(obj, x, y) {
		this.instance.emit('edit move t', [obj, t]);
	}
}
