...
class Socket {
	constructor() {
		self.instance = io.connect();
		self.instance.on("news", function(data) {
		...
		self.instance.on("connect", function() {
			...
			var delivery = new Delivery(self.instance);
				...
				var upload = function(file) {
					...
					newZip.loadAsync(file).then(function(zip) {
						...
						var temp = function(data) {
							...
							self.instance.removeListener("osu", temp);
						}
						Editor.loading = true;
						$("#loading").fadeIn();
						self.instance.on("osu", temp);
					});
					$("#fileuploadpanel").slideUp();
				};
